...
Parameter wordsize_minus_one : nat.
...
Context (wordsize_minus_one : nat).
...
Proof. unfold wordsize. intros CONTRA. inversion CONTRA. Qed.
...
Definition Zzero_ext (n: Z) (x: Z) : Z :=
  Z.iter n
    (fun rrec x => Zshiftin (Z.odd x) (rrec (Z.div2 x)))
    (fun x => 0)
    x.
...
Transparent repr.
Lemma add_repr : forall x y : Z, (add (repr x) (repr y)) = repr (x + y)%Z.
Proof.
...
Proof.
  ...
  destruct (zlt i j).
  - rewrite Z.shiftl_spec_low; auto. rewrite orb_false_l.
...
Proof.
  ...
  - rewrite Z.shiftl_spec_high.
    fold (testbit x (i + j)).
    rewrite bits_above. rewrite orb_false_l.
    unfold testbit. f_equal.
    symmetry. apply Zmod_unique with (k + 1).
    rewrite EQ. ring.
    omega. omega. omega. omega.
...
Arguments sub {_} _ _.
Arguments mul {_} _ _.
Arguments divs {_} _ _.
Arguments mods {_} _ _.
Arguments divu {_} _ _.
Arguments modu {_} _ _.
Arguments and {_} _ _.
Arguments or {_} _ _.
Arguments xor {_} _ _.
Arguments not {_} _.
Arguments shl {_} _ _.
Arguments shru {_} _ _.
Arguments shr {_} _ _.
Arguments shrx {_} _ _.
Arguments shr_carry {_} _ _.
Arguments rol {_} _ _.
Arguments ror {_} _ _.
Arguments rolm {_} _ _ _.
Arguments one_bits {_} _.
Arguments is_power2 {_} _.
Arguments cmp {_} _ _ _.
Arguments cmpu {_} _ _ _.
Arguments is_false {_} _.
