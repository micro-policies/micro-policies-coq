...
Module IntIndexed (WS : Integers.WORDSIZE) <: INDEXED_TYPE.
  Module Int := Integers.Make WS.
  Import Int.
  
  Definition t := int.
  
  Definition index := ZIndexed.index ∘ intval.
  Arguments index x /.
  
  Lemma intval_inj : forall x y : int, intval x = intval y -> x = y.
  Proof.
    intros x y Hix; generalize (eq_spec x y); intros Heq.
    unfold eq, unsigned in *.
